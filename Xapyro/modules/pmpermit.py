...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "off":
        await xapyrodb.set_pm(False)
        await message.edit("**PM Guard Deactivated**")
    if arg == "on":
        await xapyrodb.set_pm(True)
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    await xapyrodb.set_limit(int(arg))
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await xapyrodb.set_permit_message(xapyrodb.PMPERMIT_MESSAGE)
        await message.edit("**Anti_PM message set to default**.")
        return
    await xapyrodb.set_permit_message(f"`{arg}`")
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await xapyrodb.set_block_message(xapyrodb.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await xapyrodb.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await xapyrodb.get_pm_settings()
    await xapyrodb.allow_user(chat_id)
...
async def deny(client, message):
    chat_id = message.chat.id
    await xapyrodb.deny_user(chat_id)

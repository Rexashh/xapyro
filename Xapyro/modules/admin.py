...
from Xapyro import app, CMD_HELP, LOGGER
from Xapyro.helpers.pyrohelper import get_arg, get_args
...
『 **Admin Tools** 』
  ...
  `ungmute` -> Kebalikan dari apa yang dilakukan gmute.
  `pin` -> pins a message.
  `del` -> menghapus pesan.
...
async def ban_hammer(_, message: Message):
    if await CheckAdmin(message) is True:
        ...
        if reply:
            ...
            if not user:
                await message.edit("**Saya tidak bisa melarang siapa pun, bukan??**")
                return
        try:
            ...
            await message.edit(f"**Dilarang {get_user.first_name} dari obrolan.**")
        except:
            await message.edit("**Saya tidak bisa melarang pengguna ini.**")
    else:
        await message.edit("**Apakah saya seorang admin di sini?**")
